...
{
    public class PlayListNamesToMediaItemsDao<T> : ResultSetDao<T> where T : class
    {
        /// <summary>
        /// Constructor
        /// </summary>
        ...
        public PlayListNamesToMediaItemsDao(T rs, string tableName)
            : base(rs, tableName)
        {
        }
        ...
        public override T GetResultSetFromDataRow(DataRow row)
        {
            ...
            {
                // Fill the resultset
                rs.PlayListId = row.ItemArray[0].ToString();
                rs.MediaId = row.ItemArray[1].ToString();
            }
        ...
        public override bool InsertResultSet(T rs)
        {
            ...
            // insert operation.
            PlayListNamesToMediaItemsResultSet prs = rs as PlayListNamesToMediaItemsResultSet;
            if (prs != null)
            {
                retVal = InvokeAndInsert(typeof(PlayListNamesToMediaItemsResultSet), new Object[] { prs.Id, prs.PlayListId, prs.MediaId });
            }
            return retVal;
        }
    }
